...
class BkashGatewaySimulateView(View):
    ...
    def get(self, request, order_number, *args, **kwargs):
        ...
        return render(request, 'payments/bkash_gateway.html', {'order': order, 'payment': payment, 'title': f'bKash Payment Gateway - #{order.order_number}'})
    ...
    def post(self, request, order_number, *args, **kwargs):
        ...
        success = gateway.verify_payment({'order_number': order_number, 'wallet_number': wallet_number, 'pin': pin})
        if success:
...
class NagadGatewaySimulateView(View):
    ...
    def get(self, request, order_number, *args, **kwargs):
        ...
        return render(request, 'payments/nagad_gateway.html', {'order': order, 'payment': payment, 'title': f'Nagad Payment Gateway - #{order.order_number}'})
    ...
    def post(self, request, order_number, *args, **kwargs):
        ...
        success = gateway.verify_payment({'order_number': order_number, 'wallet_number': wallet_number, 'otp': otp})
        if success:
...
class BkashCallbackView(View):
    ...
    def post(self, request, transaction_id, *args, **kwargs):
        ...
        pin = request.POST.get('pin', '').strip()
        gateway = BkashPayment()
        success = gateway.verify_payment({'order_number': order.order_number, 'wallet_number': wallet_number, 'pin': pin})
        if success:
            messages.success(request, f'bKash payment of ${order.total_amount} verified successfully!')
            return redirect('orders:order_success', order_number=order.order_number)
        else:
...
class NagadCallbackView(View):
    ...
    def post(self, request, transaction_id, *args, **kwargs):
        ...
        otp = request.POST.get('otp', '').strip()
        gateway = NagadPayment()
        success = gateway.verify_payment({'order_number': order.order_number, 'wallet_number': wallet_number, 'otp': otp})
        if success:
            messages.success(request, f'Nagad payment of ${order.total_amount} verified successfully!')
            return redirect('orders:order_success', order_number=order.order_number)
        else:
