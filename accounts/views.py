...
class ProfileView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        user = request.user
        profile_form = ProfileForm(instance=user.profile, initial={'first_name': user.first_name, 'last_name': user.last_name, 'email': user.email})
        address_form = AddressForm()
        addresses = Address.objects.filter(user=user)
        recent_orders = user.orders.all().order_by('-created_at')[:5] if hasattr(user, 'orders') else []
        return render(request, 'accounts/profile.html', {'profile_form': profile_form, 'address_form': address_form, 'addresses': addresses, 'recent_orders': recent_orders, 'title': 'My Account Profile'})
